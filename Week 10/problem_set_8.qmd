...
**Answer**:
First, we have to determine the t-statistic for $\beta_1$.
$$
...
**Answer**:
We can find the RSS by first solving for the ESS through the $R^2$ formula:
$$
R^2 = \frac{ESS}{TSS}
$$
```{r}
...
**Answer**:
First, we have to determine the critical value according to the 0.05 significance level. We can find this using the degrees of freedom.
```{r}
df_03 <- 102 - 1 - 1
...
**Answer**:
```{r}
regression_05 <- lm(life_exp ~ gdp_per_cap, data = gapminder_df)
summary(regression_05)
modelsummary(regression_05,
             coef_rename = c("gdp_per_cap" = "GDP per capita (USD)"),
             statistic = c("std.error", "p.value"))
```

The intercept suggests that a country with 0 GDP per capita will *on average* be associated with a life expectancy of 69.59 years. Additionally, each 1 point increase in GDP per capita is associated with a 1.953e-04 increase in life expectancy *on average*. 
The t-value of 138.46 indicates that these observed values are highly improbable under the null environment and this is reflected in the extremely low p-value. Therefore, this finding is statistically significant. 
Although the coefficient is minuscule, the data suggests support for a positive relationship between GDP per capita and life expectancy. 